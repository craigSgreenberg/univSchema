...
class ParagraphVectorModel (override val opts: EmbeddingOpts) extends WordEmbeddingModel(opts) {
  override def process(ep: Int, rel: Int): Unit = {
        trainer.processExample(new ParagraphVectorHierarchicalSoftMaxExample(this, ep, tree(rel).codelen, tree(rel).code, tree(rel).nodeList))
  }
  override def getScore(ep: Int, rel: Int): Double = {
    ...
    for (d <- 0 until tree(rel).codelen) {
      ...
      if (tree(rel).code(d) == 1) ans = ans * exp
      else if (tree(rel).code(d) == 0) ans = ans * (1 - exp)
...
class ParagraphVectorHierarchicalSoftMaxExample(model: WordEmbeddingModel, ep: Int, codeLen:Int, code: Array[Int],nodeList:Array[Int]) extends Example {
  def accumulateValueAndGradient(value: DoubleAccumulator, gradient: WeightsMapAccumulator): Unit = {
    /*
    val contextEmbedding = new DenseTensor1(model.D, 0)
    wordContexts.foreach(context => contextEmbedding.+=(model.weights(context).value))
